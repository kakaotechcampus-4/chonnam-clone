...
- 남은 질문 하나: "다들 언제 되는지"처럼 **날짜가 없는** 요청은 별개 축인데, 트레이스에서 3/3 모두 전원고려 0/6이 나왔습니다. 선택화 후 Kana가 find_slots는 부르지만, 날짜 범위가 없어 `collect_member_schedules`가 전원 busy_rows를 못 모으기 때문입니다(에러는 아니고 빈 근거로 후보를 냄). member_names와 무관한 **날짜 축** 문제라, Kana 프롬프트에서 "날짜가 모호하면 후보를 내지 말고 되묻기"를 강제할지 여부는 다음 주차 UX 논의로 넘겨도 될지 의견 주시면 좋겠습니다.
