...
- 검증 대본을 마저 돌리다 리뷰 밖 버그를 하나 더 발견해 고쳤습니다(`trace_week06_no_slot.py`): 공통 가능 시간이 없을 때(내 종일 일정) 겹침 검증(Python)은 후보를 정상적으로 비웠는데, `decide_final_slot`(LLM)이 보류하지 않고 `00:00-01:00` 같은 시간을 **지어내는 경우가 3회 중 2회** 있었습니다. Kana 프롬프트 3)에 "검증된 후보가 비면 시간을 지어내지 말고 final_slot=null·needs_agent_selection=true로 두고 '공통 시간 없음'을 알린다"를 명시해, 보강 후 3/3 정상 보류로 안정화했습니다. (겹침 검증 코어와 payload 계약은 `fixed/`라 손대지 않고 프롬프트로만 해결.)
