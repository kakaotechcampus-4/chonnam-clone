...
## "예전에 ~라고 말한 적 있나" 질문이 search_conversation_messages 대신 search_personal_references로 라우팅됨

- 증상: "내가 예전에 어떤 게임 좋아한다고 말한 적 있었나?"처럼 지나가는 일반 대화 발화를 되짚는 질문을 하면, `search_conversation_messages`가 아니라 `search_personal_references`가 호출되고 "저장된 참고자료 중에 없습니다"라는 엉뚱한 답이 나옴.
- 원인: `week04_prompt_parts()`(`student_parts/week04_retrieve_nanas_memory.py`)의 라우팅 규칙이 "등록해둔 참고자료나 배경지식을 물으면 search_personal_references"라고만 돼 있어서, 문장에 "좋아한다"처럼 선호/배경지식 느낌의 단어가 들어가면 LLM이 `add_personal_reference`로 명시적으로 저장을 요청한 적 없는 발화까지도 참고자료 조회 대상으로 오인함. "명시적으로 저장 요청했던 내용"과 "그냥 대화 중 지나가듯 말한 내용"을 구분하는 기준이 프롬프트에 없었음.
- 해결: `week04_prompt_parts()`에 구분 기준 문장("~을 참고해/알아둬/기억해줘로 명시적으로 저장을 요청했던 내용만 search_personal_references 대상")과, 실패했던 질문을 그대로 few-shot 예시로 추가: `'내가 예전에 어떤 게임 좋아한다고 말한 적 있었나?' → search_conversation_messages`. 적용 후 같은 질문과 유사 문장("내가 예전에 좋아하는 게임 있다고 말했었나?")에 `search_conversation_messages`가 호출되고, 현재 대화는 제외한 채 이전 세션(스타듀밸리/포켓몬게임 발화)이 정상적으로 검색됨을 확인함.
