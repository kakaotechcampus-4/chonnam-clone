...
## "일정 알려줘"가 개인/그룹만 보여주고 할 일/리마인더는 빠짐

- 증상: "일정 알려줘"라고 물으면 personal_schedule/group_schedule만 조회되고, todo/reminder는 안 나옴.
  반대로 "할 일/리마인더 알려줘"라고 물으면 todo/reminder만 나와서, 한 번에 전부 보여달라는 요청이 안 됨.
- 원인: kind 분류 문제가 아니라 **테이블 분리 + tool 라우팅** 문제.
  `fixed/app_store.py`의 `save_structured_request`는 kind와 무관하게 모든 요청을 `structured_requests`
  테이블(감사 로그)에 저장하지만, 그 다음 kind별로 `schedules`(personal/group), `todos`, `reminders`
  테이블에 각각 나눠 담음. `personal_list_saved_schedules` → `list_schedules()`는 `schedules` 테이블만
  조회하므로 todo/reminder는 구조적으로 거기 없음. 반면 `list_saved_requests`는 `structured_requests`
  테이블을 보므로 kind를 안 넘기면 5종 전부 나옴 — 즉 통합 조회 tool은 이미 있었음. 문제는
  `WEEK03_TOOL_CALL_PROMPT`의 아래 줄이 "일정 보여달라"는 요청을 전부 좁은 tool로만 라우팅하고 있었던 것.
  ```python
  "사용자가 저장된 일정을 보여달라고 하면 personal_list_saved_schedules를 호출한다."
  ```
- 해결: 코드는 그대로 두고(테이블 분리는 update/delete가 schedule_id/todo_id/reminder_id로 구분되는
  구조라 의도된 설계), `WEEK03_TOOL_CALL_PROMPT`의 해당 줄을 스코프별로 쪼갬.
  ```python
  ...
  ```
  수정/삭제 목적으로 schedule_id를 확인하는 라인은 그대로 둠 — schedule 대상일 때만 해당, 이번 이슈와 무관.
  적용 후 "일정 알려줘"에 `list_saved_requests`가 kind 없이 호출돼 5종 다 나오는지, "개인 일정만
  보여줘"는 여전히 `personal_list_saved_schedules`(kind=personal_schedule)로 좁혀 조회되는지 둘 다 확인.
