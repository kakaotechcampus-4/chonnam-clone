...
**해결/답변** — 먼저 정정: 교체(= Week1 이름을 SQLite로 재배선)를 해도 **빈 결과 버그는 재발하지 않는다.** 두 조회 도구가 모두 SQLite를 읽게 되니 LLM이 어느 쪽을 골라도 올바른 데이터가 나온다. 진짜 쟁점은 **중복**이다: `personal_list_saved_schedules`는 메인 과제 필수 도구라, 여기에 Week1 이름까지 SQLite로 교체하면 **같은 일을 하는 도구가 2개** 남는다.

또한 이건 "가이드를 어겼다"기보다 **가이드 내부의 충돌**이다 — 가이드의 "누적"과 필수 과제("`personal_list_saved_schedules` 구현")를 동시에 지키면 조회 도구가 **필연적으로 중복**된다. 어떤 선택도 "깔끔한 누적"은 못 만든다(교체=중복 남김, 제거=누적 문자 어김). 나는 Week3부터 임시 메모리 조회/삭제가 **죽은 경로**라 판단해 도구 표면을 줄이는 **제거**를 택했다(생성만은 짝꿍 도구가 없어 중복이 안 생기므로 이름 유지+교체). 교체도 유효한 선택임을 인정하며, 근거는 ADR-4에 기록했다.
...
- **왜 조회/삭제는 교체가 아니라 제거인가**: `personal_list_saved_schedules`는 **메인 과제 필수 도구**라 지울 수 없다. 여기에 `personal_list_schedules`도 SQLite로 교체하면 두 도구가 모두 SQLite를 읽어 **빈 결과 버그는 안 나지만, 같은 일을 하는 도구가 2개** 남는다(중복). 즉 교체의 문제는 "버그 재발"이 아니라 "불필요한 중복"이다. 그래서 죽은 경로를 없애는 **제거**를 택했다.
...
- **가이드 충돌**: 사실 "누적"(Week1 도구 유지)과 필수 과제("`personal_list_saved_schedules` 구현")를 동시에 지키면 조회 도구가 **필연적으로 중복**된다. 어떤 선택도 깔끔한 누적은 못 만든다(교체=중복 남김, 제거=누적 문자 어김) → 가이드 위반이라기보다 **트레이드오프 판단**이다.
- **정리**: 획일적 "다 제거"가 아니라 **생성=교체(중복 없음), 조회/삭제=제거(교체하면 필수 도구와 중복)**. 교체도 유효하되 죽은 경로를 없애는 제거를 택했고 근거는 ADR-4.
...
- **대안(교체) 검토**: 멘토 제안대로 Week1 이름을 SQLite로 재배선하는 "교체"도 유효하다 — 그 경우 빈 결과 버그는 나지 않는다(둘 다 SQLite). 다만 필수 도구 `personal_list_saved_schedules`와 **같은 일을 하는 도구가 2개** 남는다. 또한 가이드의 "누적"과 필수 도구 구현을 동시에 지키면 중복이 **불가피**하므로, 이 결정은 가이드 위반이라기보다 그 충돌을 어떻게 풀지에 대한 **트레이드오프 판단**이다(교체=중복 유지 / 제거=죽은 경로 정리). 도구 표면을 줄이는 제거를 택했다.
