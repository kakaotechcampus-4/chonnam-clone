...
- 라우팅이 아니라 **tool 인자값**을 지시하는 규칙(예: "날짜 미지정 조회는
  date_from/date_to를 비운다", "집합 표현이면 member_names를 임의로 채우지
  않는다")도 같은 방식으로 찾습니다. 이런 규칙을 발견하면, args_schema에서
  그 인자 이름이 실제로 존재하는지 확인한 뒤 4단계에서 `expected_inner_args`로
  같이 검증하도록 표시해둡니다 — 이 인자가 옳은지는 라우팅 체크만으로는
  못 잡히므로 별도 표시가 없으면 다음 단계에서 누락되기 쉽습니다.
...
- **`expected_inner_args`는 발견하는 즉시 자동으로 붙입니다**: 1.4단계에서
  "tool 인자값 규칙"을 찾았다면, 그 규칙이 적용되는 프롬프트를 만들 때마다
  사람이 나중에 따로 골라 붙이지 말고 프롬프트 생성 시점에 바로
  `expected_inner_args`를 함께 씁니다. 판단 기준: 프롬프트 문구에 그 규칙의
  "조건"(예: 날짜/기간 표현)이 전혀 없으면 그 규칙의 "기본값"(보통 None 또는
  인자 자체 생략)을 `args_equal`로 명시합니다. 조건이 있으면(예: "어제 일정")
  그 값이 논리적으로 결정 가능할 때만 선언하고, 애매하면 생략합니다(잘못된
  기대값을 강제하면 정상 동작이 위반으로 오탐됩니다).
