...
## 라벨 재검토 판정 기준 (SKILL.md 6.5단계 보강)

"알려진 이력"의 원인 A~E는 사람이 매번 새로 판단하지 않도록, 아래 체크 조건을 만족할 때만 적용한다.
조건에 안 들어가면 기본값은 "실제 라우팅 약점"이다.

| 원인 | 적용 조건(전부 만족해야 함) |
|---|---|
| A. 선조회 정상 동작 | 시스템 프롬프트에 해당 조회를 선행하라는 명시 규칙이 있고, 실제 `tool_calls`에 그 조회 tool이 포함됨 |
| B. 프롬프트 설계 결함 | 프롬프트 text 자체에 대상 tool의 필수 인자(query/날짜/conversation_id 등) 중 하나 이상이 아예 없음 |
| C. 경계값 함정 | `category=boundary`이고 `tool_calls`가 빈 배열이며, 프롬프트 text에 명시적 이상값(음수/0/상한초과)이 있음 |
| D. 통합 tool 함정 | expected_tool이 통합형 tool이고, agent가 개별 tool 조합으로 호출했으며, 최종 answer 내용까지 사람이 대조해 동일함을 확인함 |
| E. 라벨 자체 애매 | 프롬프트 text가 실제 이지선다 상황이고, 두 후보 tool 모두 "정상 사용"으로 볼 근거가 `reason` 필드에 이미 적혀 있음 |

**용어 정정**: 위 5개는 전부 `expected_tool`/`reason` 필드를 실제로 고치지 않은 "관찰된 편차의 정당화"다.
25건 재검토에서 jsonl의 `reason` 필드나 `expected_tool` 값을 수정한 건 0건이었다 — SKILL.md 186-189줄이
요구하는 "원래 라벨 → 바뀐 라벨 + 근거"는 **라벨 자체를 고칠 때만** 적용되는 규칙이라 해당 없음.
"알려진 이력"에서 이 25건을 "라벨 오류로 기각"이라 부른 건 부정확한 표현이었다 — 정확히는 "라벨은 맞지만
관찰 편차가 A~E 중 하나로 설명돼 실제 버그로 세지 않음"이다.

**기준표를 25건에 실제 적용한 감사 결과**: p094는 원인 A로 분류돼 있었으나 1차 실행 `tool_calls`가 빈
배열이라("조회 tool이 실제 호출됨" 조건 불충족) A 조건을 충족하지 못한다 — 아래 "알려진 이력"에서 A 목록에서
제외하고 별도 확인 대상으로 남긴다. 나머지 24건은 기준표 조건을 충족함을 확인했다.
...
- 2026-07-29 1차 실행(20260729-195031): 91개 기대 매칭 중 58 일치/33 불일치. 6.5단계 라벨 재검토 결과:
  - **라벨 오류로 기각(25개)**: p003,p004,p009,p010,p018,p019,p027,p031,p032,p033,p034,p035,p036,p037,p038,p040,p044,p045,p063,p064,p066,p073,p075,p093,p094.
    - 원인 A (존재하지 않는 ID를 단발성으로 언급) → week03 시스템 프롬프트 규칙("삭제/수정 전 반드시 personal_list_saved_schedules로 먼저 조회, schedule_id는 추측 금지")대로 agent가 조회부터 한 게 정상: p003,p009,p010,p063,p064,p066,p093.
      (p094는 판정 기준표 감사에서 A 조건 불충족 확인 — tool_calls가 빈 배열이라 "조회부터 했다"는 근거 자체가 없음. 별도 확인 필요로 재분류.)
...
- 2026-07-29 2차 실행(20260729-204114), `week05_prompt_parts` 충돌1/충돌2 수정 반영 후: 매치 58→70(91개 중), 불일치 33→20.
  ...
  - **충돌1 오염 재검증**: 이 실행은 100개 프롬프트가 한 격리 DB를 공유해서, 앞선 프롬프트의 "철수" 언급이
    검색에 섞였을 가능성을 완전히 배제 못한다는 캐비어트가 있었다. p015/p054/p056/p070/p080을 각각
    독립된 prompts 파일 + 독립된 `--isolate-dir`로 단독 실행해 다른 프롬프트가 전혀 없는 상태에서
    재검증했다(다섯 번의 개별 harness 실행). 5개 전부 `search_previous_conversations`로 정상 라우팅됨 —
    오염 여지를 없앤 상태에서도 통과하므로, 원래 실패는 데이터 오염이 아니라 순수 라우팅 문제였고
    프롬프트 예외 문구 수정이 오염과 무관하게 유효함을 확인했다. 캐비어트 해제.
  - **충돌2 수정 확인**: p020/p067/p071 전부 extract_schedules_from_history(또는 collect_member_schedules)로 정상화.
    p019는 1차 실행에서 이미 "원인 E(라벨 애매, 실사용 문제없음)"로 결론난 케이스라 이 목록에서 제외 —
    이전 기록이 같은 케이스를 두 가지로 다르게 서술한 모순이었음.
  - **p042 기록 정정**: "충돌2 수정으로 정상화"에 p042를 포함시킨 건 오기였음. p042 라벨은
    `expected_tool: null`("member_names 불특정 - 필수 필드 부족, 되묻는 게 맞음")인데, 실행 3회
    (1차/2차/재작성본) 전부 agent가 되묻지 않고 member_names에 임의값("다른 사람들"/"다들")을 채워 호출하거나
    무관한 tool을 호출했음. 재현성 있는 실제 라우팅 약점으로 재분류 — week05_prompt_parts에 집합 표현
    ("다들"/"다 같이"/"모두")으로 멤버가 특정되지 않은 채 외부 busy-time을 물으면 임의값으로 호출하지 말고
    되묻도록 규칙 추가(아래 코드 반영).
