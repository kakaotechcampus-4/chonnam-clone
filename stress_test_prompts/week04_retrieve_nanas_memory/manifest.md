...
- 2026-07-26 r004: allow_rank 2 추가 (distance 측정 결과 '3월/4월' 구분은 임베딩
  한계 — f001 0.824 vs f002 0.873 역전. 임베딩 모델 교체 시 재검토).
- 2026-07-26 r003: 라벨 유지. 라우팅이 실행마다 흔들리는 비결정 경계 케이스로
  기록 (fix2 실행 참고자료 → fix3 실행 SQLite). 간헐 실패는 경계 모호성 신호.
- 2026-07-26 과제 코드: search_personal_reference_hits에 관련도 컷
  RELEVANCE_MAX_DISTANCE=1.2 도입 (정답 ≤1.04 / 무관 ≥1.34 측정 기반).
- 2026-07-26 과제 코드: '~한 내용' 규칙이 대화 검색(p060)까지 끌어당기는 부작용
  발견(라우팅 100건 회귀 체크) → 규칙에 "채팅/대화에서 얘기했던 것은
  search_conversation_messages 우선" 예외 추가. p060 회복, r009 유지 확인.
  참고: 7/22 두 실행 간 대조군 비교로 자연 변동(회귀 13건)과 수정 영향(실질 1건)을
  분리해 판정함 — 회귀 판정 시 대조군 비교 권장.
