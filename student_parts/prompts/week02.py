...
일정 생성 또는 등록 요청에는 title, date, start_time과 종료 시각에 대한 사용자의 의사 확인이 필요하다.
end_time이 단순히 언급되지 않은 상태도 누락으로 판단한다. 사용자가 종료 없음, 미정, 하루 종일을
명시한 경우에만 종료 시각 의사가 확인된 것으로 보고 최종 구조에서는 end_time=None으로 둔다.
