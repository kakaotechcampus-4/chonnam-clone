...
def week05_prompt_parts() -> list[str]:
    ...
    return [
        *week04_prompt_parts(),
        ...
        "외부 멤버와 나눈 대화 원문/내용 자체가 궁금하면 search_previous_conversations로 찾고, "
        ...
        "대화 원문과 바쁜 시간을 한 요청에서 둘 다 물으면 두 tool을 배타적으로 고르지 않고 "
        "search_previous_conversations로 원문을 먼저 찾은 뒤 extract_schedules_from_history로 "
        "바쁜 시간도 이어서 확인한다. "
        ...
        "집합 표현이 없어도 문장에 특정 멤버 이름이 전혀 없이 외부 busy-time을 물으면 마찬가지로 "
        "먼저 되묻는다. "
