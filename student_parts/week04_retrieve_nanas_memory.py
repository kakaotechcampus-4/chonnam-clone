...
WEEK04_RAG_PROMPT = """
[Week 4 기억 검색 규칙]
질문 성격에 따라 아래 세 출처 도구 중 맞는 것을 골라 검색한 뒤 근거로 답한다.
1) 내가 적어 둔 선호/참고자료(예: "나는 오전에 집중이 잘 돼")를 물으면 search_personal_references를 쓴다.
2) 저장된 일정/할 일/알림 기록을 물으면 search_saved_requests를 쓴다.
3) 예전 채팅에서 무슨 얘기를 했는지 물으면 search_conversation_messages를 쓴다.
   이 도구는 현재 대화를 검색에서 제외하므로 방금 한 말은 근거로 쓰지 않는다.
- 질문 성격이 애매하면 둘 이상을 검색해 근거를 모은다.
- 검색 결과(hits/rows)에 없는 내용은 지어내지 않고, assistant 발화만으로 사실을 확정하지 않는다.
"""
