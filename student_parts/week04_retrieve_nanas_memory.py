...
WEEK04_MEMORY_PROMPT = (
    "[Week 4 기억 검색] 질문 성격에 맞는 검색 tool을 출처별로 고른다.\n"
    "  1) search_personal_references(query, top_k) -> {hits:[{id, content, distance, metadata}]}\n"
    "     사용자가 적어 둔 개인 참고자료·선호·메모를 찾을 때 사용한다.\n"
    "  2) search_saved_requests(query, top_k) -> {rows:[structured_requests row, ...]}\n"
    "     SQLite에 저장된 일정/할 일/알림 구조화 기록을 찾을 때 사용한다.\n"
    "  3) search_conversation_messages(query, top_k, conversation_id?) -> "
    "{hits, rows, context, rag_backend, sync}\n"
    "     앱에 저장된 일반 채팅 발화를 찾을 때 사용한다. conversation_id를 생략하면 "
    "현재 대화는 검색에서 제외된다.\n"
    "  * assistant 발화만으로 사실을 확정하지 말고, user 발화와 저장 기록을 근거로 답한다.\n"
    "  * 참고자료·저장 기록·대화 발화는 서로 다른 출처이므로, 질문에 맞는 tool만 호출한다."
)

WEEK04_REFERENCE_ROUTING_PROMPT = (
    "[Week 4 저장 출처 구분] 사용자가 무언가를 저장해 달라고 할 때, 일정/할 일/알림처럼 "
    "날짜·시간이 딸린 사건인지 아니면 시간과 무관하게 계속 참고할 사실인지부터 구분한다.\n"
    "  * 혈액형·알레르기·선호·습관·자주 가는 곳처럼 날짜·시간이 없는 사실은 "
    "add_personal_reference로 저장한다. '참고자료로 저장해줘'라는 말이 없어도 "
    "내용 자체가 이런 성격이면 add_personal_reference를 사용한다.\n"
    "  * 날짜·시간이 있는 사건(회의, 할 일, 알림 등)만 extract_schedule_request/"
    "save_structured_request 계열로 저장한다."
)
...
def _schedule_chunk_lines(schedules: list[dict[str, Any]]) -> list[str]:
    """SQLite 일정 row를 agent가 읽기 쉬운 chunk 문자열 목록으로 바꿉니다."""

    lines: list[str] = []
    for index, schedule in enumerate(schedules, start=1):
        attendees = schedule.get("attendees")
        if attendees is None:
            attendees = _decode_attendees(schedule.get("attendees_json") or schedule.get("members_json"))
        attendee_text = ", ".join(str(name) for name in attendees) if attendees else "없음"
        title = schedule.get("title") or "제목 없음"
        date = schedule.get("date") or "날짜 미정"
        ...
        lines.append(
            f"[{index}] {title} | {date} {time_text} | 참석: {attendee_text}"
        )
    return lines


def _reference_context_lines(reference_hits: list[dict[str, Any]]) -> list[str]:
    """개인 참고자료 hit를 context 문자열 조각으로 바꿉니다."""
...
def search_nana_memory(
    ...
    if attendee:
        ...
        schedules = [
            ...
            if any(attendee_lower in str(name).lower() for name in (schedule.get("attendees") or []))
        ]
    ...
    if query_lower:
        schedules = [
            schedule
            for schedule in schedules
            if query_lower in str(schedule.get("title") or "").lower()
            or query_lower in str(schedule.get("date") or "").lower()
        ]
    schedules = schedules[:normalized_limit]

    context_lines = [
        *_reference_context_lines(reference_hits),
        "",
        "[SQLite 일정 chunk]",
    ]
    if schedules:
        context_lines.extend(_schedule_chunk_lines(schedules))
    else:
        context_lines.append("- 검색된 일정이 없습니다.")

    return json_payload(
        {
            "reference_backend": REFERENCE_STORE.backend_info(),
            "reference_hits": reference_hits,
            "schedule_chunks": schedules,
            "context": "\n".join(context_lines),
        }
    )
...
def week04_prompt_parts() -> list[str]:
    ...
    return [
        *week03_prompt_parts(),
        ...
        WEEK04_REFERENCE_ROUTING_PROMPT,
        (
            f"오늘 날짜는 {current_app_date_iso()}이다. 이번 주(Week 4)의 범위는 출처별 기억 검색이다. "
            "개인 참고자료는 search_personal_references, 저장된 일정/할 일/알림 기록은 "
            "search_saved_requests, 일반 채팅 발화는 search_conversation_messages로 찾는다. "
            "질문에 맞는 tool만 호출하고, tool 결과의 hits/rows/context를 근거로 답한다."
        ),
    ]
