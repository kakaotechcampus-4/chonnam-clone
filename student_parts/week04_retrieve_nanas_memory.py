...
from student_parts.prompts.week04 import (
...
def search_nana_memory(
    ...
    reference_hits = search_personal_reference_hits(REFERENCE_STORE, query=query, top_k=min(limit, 5))
    ...
    if reference_hits:
        for hit in reference_hits:
            ...
            context_lines.append(f"- {metadata.get('title', '')}: {hit.get('content', '')}")
    else:
    ...
    if schedules:
        for schedule in schedules:
            context_lines.append(
                f"- {schedule.get('title', '')} | {schedule.get('date', '')} {schedule.get('start_time', '')}"
                f" | 참석자: {', '.join(schedule.get('attendees') or [])}"
            )
    else:
        context_lines.append("- 조건에 맞는 저장된 일정이 없습니다.")
