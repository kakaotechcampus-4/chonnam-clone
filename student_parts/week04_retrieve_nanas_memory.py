...
8. ChromaDB 벡터 검색은 딱 맞는 결과가 없어도 가장 가까운 순으로 top_k개를 채워서 반환한다.
   hits가 비어 있지 않다고 바로 "관련 있다"고 판단하지 말고, 각 hit의 distance(작을수록 질문과 가까움)를
   함께 확인한다. 상위 hit들의 distance가 모두 멀어서 질문과 무관해 보이면, 근거가 없는 것으로 보고
   추측하지 말고 참고자료를 찾지 못했다고 답한다.
