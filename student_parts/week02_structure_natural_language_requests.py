...
class StructuredRequest(BaseModel):
    ...
    kind: RequestKind = Field(
        description=(
            "요청 종류. personal_schedule(개인 일정), "
            "group_schedule(여러 명이 참석하는 일정/회의), todo(할 일), "
            "reminder(알림), unknown(불명확) 중 하나만 사용한다."
        )
    )
    title: str | None = Field(
        default=None,
        description="일정 또는 할 일의 제목. 확실하지 않으면 None으로 둔다.",
    )
    date: str | None = Field(
        default=None,
        description="YYYY-MM-DD 형식의 날짜. 상대 표현은 base_date 기준으로 환산하고 모르면 None.",
    )
    start_time: str | None = Field(
        default=None,
        description="HH:MM 형식의 시작 시각. 확실할 때만 채우고 모르면 None.",
    )
    end_time: str | None = Field(
        default=None,
        description="HH:MM 형식의 종료 시각. '미정'처럼 시각이 아닌 값은 None으로 둔다.",
    )
    members: list[str] = Field(
        default_factory=list,
        description="참석자나 관련 멤버 이름 목록. 모르면 빈 목록으로 둔다.",
    )
    priority: str | None = Field(
        default=None,
        description="할 일 우선순위(예: high/medium/low). 판단할 수 없으면 None.",
    )
    reason: str | None = Field(
        default=None,
        description="이 종류/필드로 판단한 근거. 특별한 근거가 없으면 None.",
    )
    original_text: str = Field(
        default="",
        description="구조화 전 사용자 원문 또는 Week1 tool JSON 원본을 그대로 보존한다.",
    )
...
class StructuredRequestBatch(BaseModel):
    ...
    requests: list[StructuredRequest] = Field(
        default_factory=list,
        description="구조화된 요청 목록. 요청이 하나뿐이어도 반드시 목록 형태로 담는다.",
    )
    base_date: str = Field(
        default_factory=current_app_date_iso,
        description="상대 날짜(예: '내일', '다음 주 화요일') 해석의 기준이 되는 오늘 날짜(YYYY-MM-DD).",
    )
        ...
        [
            *week02_prompt_parts(),
            ...
            (
                ...
                "한 문장에 여러 일정/할 일/알림 의도가 섞이면 각각을 "
                "별도의 StructuredRequest로 나눈다."
            ),
            (
                "개인 일정 생성 요청에서는 personal_create_schedule tool 결과 JSON의 "
                "created_schedule을 읽어 각 필드를 채운다. attendees는 members로 매핑하고, "
                "'미정'처럼 시각이 아닌 end_time은 None으로 둔다."
            ),
        ]
    )
...
def week02_prompt_parts() -> list[str]:
    ...
    return [
        *week01_prompt_parts(),
        (
            "[Week2] 너는 사용자의 일정 요청을 구조화하는 에이전트다. "
            f"상대 날짜 해석의 기준일은 {current_app_date_iso()}이며 이 값을 최우선으로 삼는다. "
            "앞선 주차 안내에 다른 '오늘 날짜'가 있어도 상대 날짜 계산에는 이 기준일을 사용한다."
        ),
        (
            "사용자의 자연어 요청을 StructuredRequest 필드"
            "(kind/title/date/start_time/end_time/members/priority/reason/original_text)로 "
            "구조화한다. 상대 날짜는 기준일 기준 YYYY-MM-DD, 시각은 HH:MM으로 환산하고 "
            "확실하지 않은 값은 지어내지 말고 None 또는 빈 목록으로 둔다."
        ),
        (
            "Week 1 도구(personal_create_schedule 등)가 반환한 JSON을 받은 경우에는 "
            "도구를 다시 호출하지 말고 그 payload(created_schedule)를 읽어 "
            "structured_response로 만든다."
        ),
        "Week 2에서는 SQLite 저장, RAG, 외부 멤버 일정 조율을 하지 않는다. 요청 구조화까지만 한다.",
    ]
