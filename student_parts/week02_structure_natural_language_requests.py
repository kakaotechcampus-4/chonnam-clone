...
class StructuredRequest(BaseModel):
    ...
    kind: RequestKind = Field(description="요청 종류입니다. 개인 일정, 그룹 일정, 할 일, 알림, 알 수 없음 중 하나로 분류합니다.")
    title: str | None = Field(default=None, description="일정, 할 일, 알림의 짧은 제목입니다.")
    date: str | None = Field(default=None, description="확실히 해석된 날짜입니다. YYYY-MM-DD 형식으로만 채웁니다.")
    start_time: str | None = Field(default=None, description="확실히 해석된 시작 시간입니다. HH:MM 형식으로만 채웁니다.")
    ...
    reason: str | None = Field(default=None, description="이 구조화 결과를 선택한 짧은 근거입니다.")
    ...
    return [
        *week01_prompt_parts(),
        (
            ...
            f"상대 날짜 표현은 앱 현재 날짜 {current_app_date_iso()}를 기준으로 해석한다."
        ),
        (
            "사용자의 한국어 자연어 요청을 StructuredRequest 필드로 구조화한다. "
            "개인 일정은 kind='personal_schedule', 여러 사람의 공동 일정 조율은 kind='group_schedule', "
            "할 일은 kind='todo', 알림은 kind='reminder', 분류가 불확실하면 kind='unknown'을 사용한다. "
            "title, date, start_time, end_time, members, priority, reason, original_text를 가능한 범위에서 채운다."
        ),
        (
