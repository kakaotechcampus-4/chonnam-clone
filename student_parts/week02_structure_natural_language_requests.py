...
class StructuredRequest(BaseModel):
    ...
    kind: RequestKind = Field(
        description="personal_schedule, group_schedule, todo, reminder, unknown 중 하나입니다"
    )

    title: str | None = Field(default=None, description="요청 제목")
    date: str | None = Field(
        default=None, description="요청 날짜, 확실할 때만 YYYY-MM-DD 형식으로 채웁니다."
    )
    start_time: str | None = Field(
        default=None, description="시작 시간, 확실할 때만 HH:MM 형식으로 채웁니다."
    )
    end_time: str | None = Field(
        default=None, description="종료 시간, 확실할 때만 HH:MM 형식으로 채웁니다."
    )
    ...
    priority: str | None = Field(default=None, description="할 일 우선순위 필드입니다.")
    reason: str | None = Field(default=None, description="판단 근거 필드 입니다")
...
class StructuredRequestBatch(BaseModel):
    ...
    requests: list[StructuredRequest] = Field(
        default_factory=list,
        description="StructuredRequest 목록을 담고, 요청이 하나뿐이어도 list 형태를 유지합니다",
    )
    base_date: str = Field(
        default_factory=current_app_date_iso,
        description="현재 날짜를 기준으로 계산합니다. ",
    )
...
def extract_structured_request(text: str) -> StructuredRequest:
    ...
    # agent loop를 새로 만들지 않고 chat_model().with_structured_output(...)만 사용해서
    # 자연어 또는 JSON 문자열을 StructuredRequest로 검증/구조화
    join_system_prompt(week02_prompt_parts())
    results = structured_LLM.invoke(
        [("system", join_system_prompt(week02_prompt_parts())), ("human", text)]
    )
    # 시스템 메시지에는 week02_prompt_parts()를 넣고, user 메시지에는 text를 넣어 invoke

    return _coerce_structured_request(results)
    # 결과가 올바른 형식인지 점검
...
def week02_system_prompt() -> str:
    ...
    return join_system_prompt(
        [
            *week02_prompt_parts(),
            """
    최종답변은 StructuredRequestBatch 형식의 structured_response로 반환하라.
    StructuredRequestBatch에는 요청이 하나만 들어와도 list를 유지하고 requests 목록에  StructuredRequest 하나를 담아라. 
    base_date는 현재 날짜를 넣는다. 
    """,
            """
    personal_create_schedule의 tool 결과 JSON의 created_schedule을 읽어 필드를 채우도록 한다. 
    """,
        ]
    )
...
def week02_prompt_parts() -> list[str]:
    ...
    return [
        *week01_prompt_parts(),
        f"""
        너는 2주차 요청 구조화 agent다.
        현재 날짜는 {current_app_date_iso()}이다. "내일", "일주일 뒤" 같은 표현은 현재 날짜를 기준으로 계산한다.
        
        """,
        """
        사용자의 자연어 요청은 StructuredRequest 필드 (kind/title/date/start_time/end_time/members/priority/reason/original_text)에 맞게채워서
        StructuredRequestBatch로 구조화해라.  
        """,
        """
        Week1 tool JSON을 받은 경우, 다시 tool을 호출하지 않고 그 JSON의 payload를 읽어 StructuredRequest 값을 채워 structured_response로 만들어라.
        """,
        """Week 2에서는 SQLite에 저장하지 않고, RAG를 수행하지 않고, 외부 멤버 일정 조율을 하지 않는다. 
    """,
    ]
