...
class StructuredRequest(BaseModel):
    ...
    title: str | None = Field(default=None, description="일정이나 할 일 제목")
    date: str | None = Field(default=None, description="일정 날짜 (확실한 경우에만 YYYY-MM-DD 형식으로 작성)")
    start_time: str | None = Field(default=None, description="시작 시간 (확실한 경우에만 HH:MM 형식으로 작성)")
    end_time: str | None = Field(default=None, description="종료 시간 (확실한 경우에만 HH:MM 형식으로 작성)")
    members: list[str] = Field( default_factory=list, description="참석자")
    priority: str | None = Field(default=None, description="할 일의 우선순위")
    reason: str | None = Field(default=None, description="이 요청을 파악하거나 일정을 생성한 판단 근거")
    original_text: str = Field(default="", description="사용자가 입력한 원본 요청 텍스트 내용")
...
class StructuredRequestBatch(BaseModel):
    ...
    requests: list[StructuredRequest] = Field(default_factory=list, description="추출된 구조화 요청들의 목록")
    base_date: str = Field(default_factory=current_app_date_iso, description="상대 날짜(예: '내일', '다음 주')를 해석할 때 기준이 되는 현재 날짜 (YYYY-MM-DD 형식)")
...
def week02_system_prompt() -> str:
    ...
    final_instruction = (
        "최종 답변은 반드시 StructuredRequestBatch 형식으로 반환해야 합니다. "
