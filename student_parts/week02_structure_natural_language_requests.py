...
class StructuredRequest(BaseModel):
    ...
    kind: RequestKind = Field(default="unknown", description="요청의 종류")
    title: str | None = Field(default=None, description="제목")
    date: str | None = Field(default=None, description="일정 날짜, YYYY-MM-DD")
    start_time: str | None = Field(default=None, description="시작시간, HH:MM")
    end_time: str | None = Field(default=None, description="종료시간, HH:MM")
    members: list[str] = Field(default_factory=list, description="일정에 속한 인원들")
    priority: PriorityKind | None = Field(
        default=None, description="할 일 우선순위, 높음/보통/낮음"
    )
    # str 대신 좀 더 우선순위를 구조화 하기 위해 PriorityKind를 만들어 넣었습니다
    reason: str | None = Field(default=None, description="판단 근거")
    original_text: str = Field(default="", description="사용자가 입력한 원본 문장을 그대로 저장")
...
class StructuredRequestBatch(BaseModel):
    ...
    requests: list[StructuredRequest] = Field(
        ...
        description="StructuredRequest 목록, 요청이 하나뿐이라도 list에 담을 것",
    )
    base_date: str = Field(
        default_factory=current_app_date_iso, description="상대 날짜 해석 기준일"
    )
...
def week02_system_prompt() -> str:
    ...
    return join_system_prompt(
        [
            *week02_prompt_parts(),
            "규칙1: 최종 답변은 StructuredRequestBatch 형식으로 내라",
            "규칙2: 요청이 하나여도 requests 리스트에 하나 담아라",
            "규칙3: personal_create_schedule 결과의 created_schedule JSON을 읽어 필드를 채워라",
        ]
    )
...
def week02_prompt_parts() -> list[str]:
    ...
    return [
        *week01_prompt_parts(),
        f"너는 Week 2 요청 구조화 agent다. 오늘은 {current_app_date_iso()}다",
        "자연어를 StructuredRequest 필드로 구조화해라",
        "Week 1 tool JSON을 받으면 tool을 다시 부르지 말고 그 payload를 읽어서 structured_response를 만들어라",
