...
class StructuredRequest(BaseModel):
    ...
    date: str | None = Field(default=None, description="요청과 관련된 날짜. 확실할 때만 YYYY-MM-DD 형식으로 채운다.")
    start_time: str | None = Field(default=None, description="시작 시각. 확실할 때만 HH:MM 형식으로 채운다.")
    end_time: str | None = Field(default=None, description="종료 시각. 확실할 때만 HH:MM 형식으로 채운다.")
    ...
    reason: str | None = Field(default=None, description="이 값들을 그렇게 판단한 근거.")
    original_text: str = Field(default="", description="구조화하기 전 원문 텍스트 보존용 필드.")
...
class StructuredRequestBatch(BaseModel):
    ...
    requests: list[StructuredRequest] = Field(
        default_factory=list, description="자연어에서 추출한 StructuredRequest 목록. 요청이 하나뿐이어도 list 형태를 유지한다."
    )
    base_date: str = Field(
        default_factory=current_app_date_iso, description="상대 날짜 표현(내일, 다음 주 화요일 등)을 해석하는 기준일(YYYY-MM-DD)."
    )
...
def week02_prompt_parts() -> list[str]:
    ...
    return [
        *week01_prompt_parts(),
        (
            f"너는 Week 2 요청 구조화 담당이다. 오늘 날짜는 {current_app_date_iso()}이며, "
            "이 날짜를 상대 날짜(내일, 다음 주 화요일 등) 해석 기준일로 사용한다.\n"
            "사용자의 자연어 요청 또는 Week 1 tool이 반환한 JSON payload를 읽고, "
            "kind/title/date/start_time/end_time/members/priority/reason/original_text 필드를 채운 "
            "StructuredRequest로 구조화한다. 한 문장에 여러 일정/할 일/알림 의도가 섞여 있으면 "
            "각각을 별도의 StructuredRequest로 나눈다. 확실하지 않은 값은 억지로 만들지 말고 "
            "None 또는 빈 list로 둔다.\n"
            "personal_create_schedule tool을 호출해 개인 일정을 생성한 경우, 그 결과 JSON의 "
            "created_schedule 필드를 읽어 StructuredRequest 필드를 채우면 된다. 이미 필요한 정보가 "
            "tool 결과에 있다면 같은 목적으로 tool을 다시 호출하지 않는다.\n"
            "최종 답변은 항상 StructuredRequestBatch 형태의 structured_response로 반환한다. "
