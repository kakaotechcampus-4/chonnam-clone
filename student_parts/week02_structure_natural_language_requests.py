...
- 이런 경우 start_time을 채우지 말고 kind를 unknown으로 둔 채, reason에 오전/오후 중 언제인지
  되묻는 질문을 남깁니다. '저녁 11시'처럼 시간대 표현이 함께 있거나 13시부터 23시까지(또는 0시)처럼
  24시간제로 명확히 말한 경우는 그대로 사용합니다.
- '24시'라고 말하면 그대로 24:00으로 채우지 않습니다. HH:MM 형식에 24:00은 없고 다음 날 00:00과
  같은 시각이라 date가 하루 밀릴 수 있으므로, 오늘 자정(24시)인지 다음 날 0시인지를 kind=unknown으로
  두고 reason에 되묻습니다."""
