...
def week02_system_prompt() -> str:
    ...
    return join_system_prompt([
        *week02_prompt_parts(),
        ...
        "- tool 결과가 ok=false이면 일정이 생성되지 않은 것이다. 값을 지어내서 채우지 말고 "
        "error 내용을 사용자에게 그대로 전달하며, 해당 요청은 확실한 필드만 채우고 나머지는 None으로 둔다.",
    ])
...
def week02_prompt_parts() -> list[str]:
    ...
    return [
        *week01_prompt_parts(),
        ...
        "## Tool 결과 처리\n"
        "- Week 1 tool 결과 JSON을 이미 받은 경우 같은 tool을 다시 호출하지 말고, "
