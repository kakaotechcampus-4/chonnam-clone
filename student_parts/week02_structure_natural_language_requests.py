...
class StructuredRequest(BaseModel):
    ...
    kind: RequestKind = Field(
        description=(
            ...
            "- 1순위 reminder: 미래의 특정 시점에 알림을 울려달라는 명시적 주문이면 "
            '"~까지" 마감 표현이 함께 있어도 reminder. "알려줘", "리마인드", "알람 걸어줘", '
            '"까먹지 않게" 같은 알림 요청 표현이 있고, 그 표현이 알림이 울릴 시점의 예약을 '
            "가리킬 때만 해당한다.\n"
            '  예: "<날짜> <시각>에 <일정> 있다고 미리 알려줘", "<시각>에 <할 일> 까먹지 않게 해줘", '
            '"<요일>까지 <문서> 마감 알려줘" → reminder\n'
            '  반례: "<시점> <정보> 알려줘"는 지금 답해달라는 정보 요청이라 알림 예약 주문이 아니다 '
            '→ unknown. "까먹을 것 같아" 같은 걱정 진술도 알림 주문이 아니다 — 할 일이 명확하면 '
            "todo로 판정하고 reason에 알림 의도가 애매함을 남긴다.\n"
            "- 2순위 todo: 특정 시각의 약속이 아니라 마감이 있거나 완료해야 할 작업이면 todo. "
            "알림 예약 주문 없이 마감 표현만 있으면 todo.\n"
            '  예: "<요일>까지 <문서> 제출해야 해", "할 일 목록에 <할 일> 추가해줘" → todo\n'
            "- 3순위 group_schedule: 여러 사람의 가능 시간을 조율하거나 팀/모임 공동 일정을 잡는 요청이면 "
            "group_schedule.\n"
            '  예: "<사람>들이랑 <모임> 날짜 좀 잡아줘. 다들 되는 시간으로" → group_schedule\n'
            "- 4순위 personal_schedule: 특정 날짜/시각에 나의 일정을 만드는 요청이면 personal_schedule. "
            "참석자 이름이 언급되어도 시간이 이미 정해진 내 일정이면 personal_schedule이고 "
            "members에 이름을 담는다.\n"
            '  예: "<날짜> <시각>에 <사람>이랑 <일정> 잡아줘" → personal_schedule\n'
...
def week02_prompt_parts() -> list[str]:
    ...
    return [
        *week01_prompt_parts(),
        ...
        "- 알림 예약을 명시적으로 주문했으면 마감 표현이 함께 있어도 reminder이고, "
        "알림 예약 주문 없이 마감 표현만 있으면 todo다.\n"
