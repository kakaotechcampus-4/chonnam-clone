...
class StructuredRequest(BaseModel):
    ...
    kind: RequestKind = Field(description="Nana가 처리할 요청 종류. 개인 일정, 그룹 일정, 할 일, 알림, 알 수 없음 중 하나")
    title: str | None = Field(default=None, description="일정 제목이나 할 일 이름처럼 요청을 짧게 나타내는 말")
    date: str | None = Field(default=None, description="일정일이나 마감일. 알 수 있으면 YYYY-MM-DD로 적고, 모르면 비워 둠")
    start_time: str | None = Field(default=None, description="일정 시작 시간. 알 수 있으면 HH:MM으로 적고, 모르면 비워 둠")
    end_time: str | None = Field(default=None, description="일정 종료 시간. 알 수 있으면 HH:MM으로 적고, 모르면 비워 둠")
    members: list[str] = Field(default_factory=list, description="일정 참석자나 요청에 언급된 사람들")
    priority: str | None = Field(default=None, description="할 일이나 알림의 우선순위가 드러난 경우에만 적음")
    reason: str | None = Field(default=None, description="요청을 이 종류로 분류한 이유")
    original_text: str = Field(default="", description="구조화의 바탕이 된 사용자 원문")
...
class StructuredRequestBatch(BaseModel):
    ...
    requests: list[StructuredRequest] = Field(
        ...
        description="사용자 문장에서 뽑아낸 Nana 요청 목록. 요청이 하나여도 리스트에 담음",
    )
    base_date: str = Field(default_factory=current_app_date_iso, description="오늘, 내일, 다음 주 같은 표현을 해석할 때 기준이 되는 날짜")
...
def week02_system_prompt() -> str:
    ...
    return join_system_prompt(
        [
            *week02_prompt_parts(),
            (
                "최종 응답은 반드시 StructuredRequestBatch 형식의 structured_response로 반환한다. "
                "요청이 하나뿐이라도 requests 목록 안에 StructuredRequest를 하나 넣는다. "
