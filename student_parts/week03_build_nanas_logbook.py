...
SQLITE_MEMORY_PROMPT = """## Week 3 · 영속 메모리
- Week 1의 개인 일정은 이번 대화가 끝나면 사라지는 임시 메모였지만, Week 3부터는 personal_create_schedule로
  만든 일정과 save_structured_request로 저장한 요청이 앱 DB(SQLite)에 남습니다.
- 대화가 새로 시작되거나 앱을 다시 켜도 이전에 저장한 일정/할 일/알림은 사라지지 않습니다. "예전에 저장한
  일정 있나요?" 같은 질문에는 새로 지어내지 말고 조회 tool로 실제 저장 내용을 확인한 뒤 답합니다."""
...
def _save_input_from(value: SaveStructuredRequestInput | StructuredRequest | dict[str, Any] | str) -> SaveStructuredRequestInput:
    ...
    raise RuntimeError(f"예상하지 못한 저장 입력 형태입니다: {type(value)!r}")
    ...
    *,
    ...
    if not delete_all and not any([schedule_ids, date, title, start_time, time_unspecified]):
        return tool_result(
            ...
            ok=False,
            reason="삭제 조건이 없습니다. schedule_ids나 날짜/제목/시간 필터, 또는 delete_all=True를 지정하세요.",
            filters=filters,
        )
    ...
    if result is None:
        return json_payload(
            tool_result(
                ...
                reason="해당 schedule_id를 찾을 수 없습니다.",
            )
        )
    return json_payload(
        tool_result(
...
def week03_prompt_parts() -> list[str]:
    ...
    return [
        *week02_prompt_parts(),
        f"""## Week 3 · 자연어 요청을 SQLite 기록으로 저장

오늘 날짜는 {today}입니다. Week 2에서 만든 StructuredRequest 구조화 결과를 이번 주차부터는 대화가 끝나도
남는 앱 DB(SQLite) 기록으로 이어붙입니다. 구조화 규칙은 Week 2와 동일하고, 그 결과를 저장·조회·수정·삭제할
수 있게 하는 것이 Week 3에서 새로 추가되는 범위입니다.""",
        SQLITE_MEMORY_PROMPT,
        WEEK03_TOOL_CALL_PROMPT,
        f"""## Week 3 · tool 선택 기준과 범위
- 사용자의 말이 새 저장 요청인지, 기존 저장 내용의 조회/수정/삭제 요청인지를 먼저 구분하고 그에 맞는
  tool을 고릅니다.
- 오늘 날짜({today}) 기준 상대 날짜 계산 규칙은 Week 2와 동일합니다.
- 이번 주차에서는 RAG 검색이나 외부 멤버와의 일정 조율은 다루지 않습니다. SQLite 저장·조회·수정·삭제까지가
  Week 3의 범위입니다.""",
    ]
