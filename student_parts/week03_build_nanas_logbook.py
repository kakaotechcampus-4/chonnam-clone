...
삭제 요청: 먼저 personal_list_saved_schedules로 후보를 확인한 뒤
  personal_delete_saved_schedules에 schedule_ids 또는 명시 필터를 넘긴다. 조건 없이 삭제하지 않는다.
"""
...
def _drop_none(payload: dict[str, Any]) -> dict[str, Any]:
    """값이 None인 필드를 뺀 저장용 dict를 만듭니다.

    저장 tool들이 공통으로 쓰던 dict comprehension을 한 곳으로 모읍니다.
    저장 대상 필드를 명시된 dict로만 받으므로, locals()처럼 의도치 않은
    지역변수가 payload에 섞이지 않습니다.
    """
...
def _save_input_from(
    ...
    if isinstance(value, str):
        ...
        try:
            ...
            data = extract_structured_request(text).model_dump()
...
def save_structured_request_payload(
    ...
    *,
    ...
    payload = _drop_none(data.model_dump())
    saved = (store or _store()).save_structured_request(payload)
    return tool_result("save_structured_request", save=saved)
...
def _delete_saved_schedules(
    *,
    ...
    has_filter = (
        delete_all or bool(schedule_ids) or any([date, title, start_time, time_unspecified])
    )
    if not has_filter:
        return tool_result(
            "personal_delete_saved_schedules",
            ok=False,
            error="삭제 조건이 필요합니다. schedule_ids나 날짜/제목/시간 필터, 또는 delete_all을 지정하세요.",
            deleted_count=0,
            filters=filters,
            deleted=[],
        )
...
def personal_create_schedule(
    ...
    payload = _drop_none(save_input.model_dump())
...
def save_structured_request(
    ...
    saved = _store().save_structured_request(payload)
    return json_payload({"ok": True, "tool_name": "save_structured_request", "save": saved})
...
def week03_prompt_parts() -> list[str]:
    ...
    return [
        *week02_prompt_parts(),
        "Week 2에서 만든 StructuredRequest는 이제 임시 메모리가 아니라 save_structured_request로 SQLite에 저장한다.",
        SQLITE_MEMORY_PROMPT,
        WEEK03_TOOL_CALL_PROMPT,
        f"너는 Week 3 기록장 agent다. 오늘은 {current_app_date_iso()}다.",
        "저장은 save_structured_request, 조회는 list_saved_requests/get_saved_request/personal_list_saved_schedules, "
        "수정은 personal_update_saved_schedule, 삭제는 personal_delete_saved_schedules를 쓴다.",
        "Week 3에서는 SQLite 저장/조회/수정/삭제까지만 다루고, RAG나 외부 멤버 일정 조율은 하지 않는다.",
    ]
