...
SQLITE_MEMORY_PROMPT = (
    "Week 1의 개인 일정은 대화가 끝나면 사라지는 임시 메모리이지만, Week 3부터는 구조화된 요청을 "
    "SQLite 앱 DB에 저장하므로 대화가 끝나거나 새 대화를 시작해도 내용이 남는다. 사용자가 이전에 "
    "저장한 일정/할 일/알림을 물으면 메모리가 아니라 personal_list_saved_schedules나 "
    "list_saved_requests/get_saved_request로 SQLite를 조회해 답한다."
)

WEEK03_TOOL_CALL_PROMPT = (
    "일정/할 일/알림을 저장해야 하는 자연어 요청이 들어오면 먼저 extract_schedule_request로 구조화한 뒤, "
    "반환된 structured_request의 필드를 save_structured_request의 같은 이름 인자에 그대로 전달해 호출한다. "
    "저장 결과의 request_id와 saved_rows를 사용자에게 자연어로 확인해 준다. 저장된 일정을 보여달라는 "
    "요청에는 personal_list_saved_schedules를 호출해 목록으로 답한다."
)
...
def week03_prompt_parts() -> list[str]:
    ...
    return [
        *week02_prompt_parts(),
        ...
        WEEK03_TOOL_CALL_PROMPT,
        (
            f"오늘 날짜는 {current_app_date_iso()}이다. 이번 주(Week 3)의 범위는 구조화 저장과 조회이며, "
            "personal_create_schedule/personal_update_saved_schedule/personal_delete_saved_schedules는 "
            "아직 구현되지 않았으므로 호출하지 않는다. 일정을 새로 만들거나 저장할 때는 항상 "
            "extract_schedule_request로 구조화한 뒤 save_structured_request를 호출한다."
        ),
    ]
