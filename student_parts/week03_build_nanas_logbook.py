...
SQLITE_MEMORY_PROMPT = (
    "Week 3부터 개인 일정/할 일/알림은 대화가 끝나도 사라지지 않고 앱의 SQLite DB에 영구 저장된다. "
    "Week 1의 임시 메모리와 달리, 새 대화를 시작했거나 앱을 재시작한 뒤에도 사용자가 예전에 저장한 "
    "일정/할 일/알림을 물으면 반드시 SQLite 조회 도구(list_saved_requests, get_saved_request, "
    "personal_list_saved_schedules)로 먼저 확인한 뒤 답한다. "
    "확인 없이 기억나는 대로 답하거나 저장된 적 없는 내용을 지어내지 않는다. "
    "조회 결과가 비어 있으면 저장된 기록이 없다고 있는 그대로 답한다."
)

# TODO: 자연어 구조화 → SQLite 저장과 조회/수정/삭제 tool 호출 순서를 안내하는 규칙을 작성하세요.
WEEK03_TOOL_CALL_PROMPT = (
    "저장된 일정을 수정·삭제하기 전에는 반드시 personal_list_saved_schedules로 먼저 조회해 실제 "
    "schedule_id를 확인한다(삭제는 schedule_id들 또는 제목/날짜 등 명확한 필터도 가능). schedule_id는 "
    "절대 추측하거나 지어내지 않는다. 조회 결과에 조건에 맞는 후보가 2개 이상이면(날짜로 좁혀도 여러 개 "
    "남는 경우, '몇 번째'/'가장 오래된' 같은 순번 지칭 포함) 임의로 하나를 골라 실행하지 않고 후보를 "
    "보여준 뒤 사용자에게 확인받는다. 조회 결과가 비어 있으면 다른 일정으로 대신 추측하지 않고 없다고 "
    "답한다. "
    "삭제는 조건이 명확할 때만 실행한다. '있으면 지워줘'처럼 존재 확인과 삭제를 묶은 요청을 포함해 "
    "대상이 불명확하면 조회만 하고, 사용자가 어떤 일정인지 구체적으로 확인해준 뒤에만 "
    ...
    "쓴다. "
    "저장 일정 조회는 personal_list_saved_schedules, 원본 구조화 기록 조회는 list_saved_requests/"
    "get_saved_request를 쓴다. personal_list_schedules·personal_delete_schedule은 현재 세션 임시 기록만 "
    "보이는 Week 1 tool이라 저장된 일정 확인/삭제에는 쓰지 않는다. "
    "'지난 일정'을 물으면 date_to를 어제가 아닌 오늘로 조회하고, 오늘 일정은 각 start_time을 현재 시각과 "
    "비교해 이미 지난 것만 지난 일정으로 판단한다."
)
    ...
    *,
    ...
    if not has_condition:
        return tool_result(
            ...
            filters=filters,
            deleted=[],
            error="삭제 조건이 없습니다. schedule_ids 또는 date/title/start_time/time_unspecified/delete_all 중 하나 이상을 지정하세요.",
        )
