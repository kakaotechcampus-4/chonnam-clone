...
SQLITE_MEMORY_PROMPT = "대화가 바뀌어도 이전 기록을 기억해야 하므로 조회 질문에는 임시 메모리가 아니라 SQLite 조회 도구를 사용한다."

# TODO: 자연어 구조화 → SQLite 저장과 조회/수정/삭제 tool 호출 순서를 안내하는 규칙을 작성하세요.
WEEK03_TOOL_CALL_PROMPT = (
    "새 요청을 저장할 때는 먼저 extract_schedule_request로 자연어를 StructuredRequest로 구조화한 뒤 "
    "그 결과의 structured_request 필드를 save_structured_request에 그대로 전달한다. "
    "저장된 일정 조회는 personal_list_saved_schedules를 사용한다. "
    "수정과 삭제 전에는 personal_list_saved_schedules로 대상 schedule_id를 먼저 확인하고, "
    "수정은 personal_update_saved_schedule, 삭제는 personal_delete_saved_schedules를 사용한다."
)
...
def _save_input_from(value: SaveStructuredRequestInput | StructuredRequest | dict[str, Any] | str) -> SaveStructuredRequestInput:
    ...
    if isinstance(value, str):
        ...
        return SaveStructuredRequestInput.model_validate(parsed_value)

    raise TypeError("지원하지 않는 저장 요청 입력입니다.")
...
def save_structured_request_payload(
    ...
    *,
    ...
    saved = target_store.save_structured_request(request_data)
    ...
    *,
    ...
    if not delete_all and not has_filter:
        return tool_result(
            ...
            ok=False,
            error="삭제할 일정의 ID 또는 날짜/제목/시간 조건이 필요합니다.",
            deleted_count=0,
            filters={},
            deleted=[],
        )
...
def save_structured_request(
    ...
    saved = _store().save_structured_request(request_data)
...
def delete_saved_schedules_dict(
    ...
    target_store = app_store or _store()
...
def personal_update_saved_schedule(
    ...
    result = _store().update_schedule(schedule_id=schedule_id, **changes)
...
def week03_prompt_parts() -> list[str]:
    ...
    return [
        *week02_prompt_parts(),
        ...
        "일정, 할 일, 알림을 새로 저장해 달라는 요청은 구조화 후 save_structured_request를 호출한다.",
        "저장된 일정 확인은 personal_list_saved_schedules, 수정은 personal_update_saved_schedule, 삭제는 personal_delete_saved_schedules를 사용한다.",
        "Week 3에서는 SQLite 저장과 저장된 일정의 조회, 수정, 삭제까지만 수행한다.",
    ]
