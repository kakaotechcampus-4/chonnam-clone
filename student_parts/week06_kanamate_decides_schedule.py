...
def week06_prompt_parts() -> list[str]:
    ...
    return [
        *week05_prompt_parts(),
        ...
        "라우팅 판단은 '날짜·시간이 이미 정해져 있는지'가 아니라 '나 외의 다른 사람이 요청에 등장하는지'로 "
        "한다. 이름이든 '팀원들'/'다 같이' 같은 집합 표현이든 나 말고 다른 사람이 하나라도 등장하면, "
        "날짜와 시간이 이미 구체적으로 주어졌어도 반드시 kana_agent를 먼저 호출해 그 사람들의 가능 시간부터 "
        "확인한다. 다른 사람의 일정을 확인하지 않고 nana_agent가 곧바로 개인 일정으로 만들면 안 된다. "
        "kana_agent가 최종 시간을 확정해 돌려주면 그 결과를 nana_agent에게 넘겨 저장을 위임한다. 요청에 나 "
        "외의 다른 사람이 전혀 등장하지 않을 때만 nana_agent에게 바로 위임한다.",
    ]
