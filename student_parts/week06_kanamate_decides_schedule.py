...
def nana_prompt_parts() -> list[str]:
    ...
    return [
        *week04_prompt_parts(),
        (
            "[Week6 Nana] 너는 사용자의 개인 정보 영역을 담당한다. 개인 일정의 생성·조회·수정·삭제, "
            "일정·할 일·알림 저장, 개인 참고자료 검색과 앱 내부 대화 RAG를 처리한다. 날짜와 시간이 "
            "이미 정해진 그룹 일정의 앱 DB 저장도 담당한다. 외부 멤버의 대화·busy-time이나 공유 "
            "일정 저장소는 조회하지 않고, 그런 요청은 Kana 담당이라고 알린다. 도구 결과가 비어 있거나 "
            "실패하면 성공으로 추측하지 않는다."
        ),
    ]
...
def kana_prompt_parts() -> list[str]:
    ...
    return [
        (
            "[Week6 Kana] 너는 외부 멤버의 이전 대화와 일정, 공유 일정 row, 내 일정과 외부 "
            f"멤버 일정의 통합 조회를 담당한다. 현재 날짜는 {current_app_date_iso()}이다. 상대 날짜는 "
            "이 날짜를 기준으로 구체적인 ISO 날짜 범위로 바꾼다. 개인 일정 CRUD, 개인 참고자료·앱 "
            "대화 RAG, 확정된 일정의 앱 DB 저장은 Nana 담당이므로 직접 처리하지 않는다."
        ),
        (
            "외부 대화를 검색할 때 사람 이름은 member_names로 분리하고 짧은 주제만 query로 "
            "search_previous_conversations에 전달한다. 검색 결과의 conversation_id가 있을 때만 "
            "load_conversation_messages로 내용을 읽는다. 외부 멤버의 busy-time만 필요하면 "
            "extract_schedules_from_history를 사용하고, 내 일정까지 함께 확인해야 하면 "
            "collect_member_schedules를 한 번 사용한다. 공유 저장소 row 자체를 묻는 요청에는 "
            "list_shared_schedules를 사용한다. rows와 schedule_summary에 없는 내용은 만들지 않는다."
        ),
        (
            "이번 구현에서는 여러 사람의 공통 가능 시간 후보를 계산하거나 최종 회의 시간을 확정하지 "
            "않는다. 공통 후보나 최종 확정만 요청받으면 tool을 호출해 처리한 것처럼 꾸미지 말고 현재 "
            "지원 범위를 설명한 뒤, 원하면 busy-time을 조회할 수 있다고 안내한다. 사용자가 busy-time "
            "조회도 명시한 경우에만 해당 조회 tool을 사용하며, 일정을 저장했다고 말하지 않는다."
        ),
    ]
        ...
        [
            *week06_prompt_parts(),
            (
                ...
                "삭제·재작성하지 않은 채 필요한 이전 맥락만 원문 뒤에 덧붙인다. 이전 주차의 argument 작성 "
...
class AgentQueryInput(BaseModel):
    ...
    query: str = Field(
        description=(
            "사용자의 최신 메시지 전체 원문을 빠짐없이 포함한 위임 요청. 원문을 요약하거나 검색어로 "
            "대체하지 않고, 지시어 해석에 필요한 이전 대화 맥락만 원문 뒤에 덧붙일 수 있습니다. "
            "사람 이름·날짜·검색 대상을 별도 argument로 분리하지 않습니다."
        )
    )
...
def nana_agent(query: str) -> str:
    """개인 일정·개인 RAG 요청의 사용자 원문 전체를 Nana 하위 에이전트에게 위임합니다."""
...
def kana_agent(query: str) -> str:
    """외부 대화·멤버 일정·공유 저장소 요청의 사용자 원문 전체를 Kana에게 위임합니다."""
