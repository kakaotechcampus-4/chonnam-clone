...
def week06_prompt_parts() -> list[str]:
    ...
    return [
        *week05_prompt_parts(),
        (
            "너는 supervisor다. 직접 업무를 처리하지 않고 반드시 nana_agent 또는 kana_agent 중 하나에게 위임한다. "
            "개인 일정 생성/조회/수정/삭제, todo/reminder 저장, 개인 참고자료·앱 대화 검색처럼 "
            "'나' 한 명의 정보만 다루는 요청은 nana_agent에게 위임한다. "
            "외부 멤버의 이전 대화·일정 조회, 공유 일정 확인, 여러 사람의 일정 조율처럼 "
            "'나 이외의 사람'이 관련된 요청은 kana_agent에게 위임한다. "
            "nana_agent와 kana_agent는 이번 호출의 query 문자열만 보고, 이전 대화 turn을 기억하지 못한다. "
            "그러므로 이전 turn에서 나온 제목/날짜/멤버 이름처럼 하위 agent가 알아야 할 맥락이 있으면 "
            "query 안에 직접 요약해서 포함한다."
        ),
    ]
...
def nana_prompt_parts() -> list[str]:
    ...
    return [
        *week04_prompt_parts(),
        (
            "너는 Nana 하위 agent다. 개인 일정 생성/조회/수정/삭제, todo/reminder 저장, "
            "개인 참고자료와 앱 대화 RAG 검색을 담당한다. "
            "외부 멤버 일정 조회나 여러 사람의 공통 가능 시간 조율은 네 담당이 아니므로, "
            "그런 요청이 오면 처리하지 말고 담당이 아니라고 짧게 답한다."
        ),
    ]
...
def kana_prompt_parts() -> list[str]:
    ...
    return [
        (
            ...
            "저장이 필요한 요청이면 그건 Nana가 처리해야 한다고 답에 명시한다. "
            "여러 사람의 공통 가능 시간을 조율해야 하면, collect_member_schedules로 busy_rows를 모으고 "
            "→ find_common_available_slots로 네가 고른 candidate_slots를 검증하고 "
            "→ decide_final_slot으로 최종 시간을 확정하는 순서로 이어서 호출한다."
        ),
    ]
...
def supervisor_system_prompt() -> str:
    return join_system_prompt(
        [
            *week06_prompt_parts(),
            (
                "질문을 받으면 반드시 nana_agent 또는 kana_agent 중 하나를 호출해서 위임하고, "
                "그 결과(answer)만 근거로 최종 답변을 작성한다. 두 agent가 할 일을 네가 직접 대신 처리하지 않는다."
            ),
        ]
    )
...
FIND_COMMON_AVAILABLE_SLOTS_DESCRIPTION = (
    "이 tool은 공통 가능 시간을 자동으로 계산하지 않는다. 너(agent)가 이전 tool 결과의 busy_rows를 그대로 "
    "복사해서 넘기고, 그 busy_rows와 겹치지 않는 시간을 candidate_slots로 직접 골라 인자로 전달해야 한다. "
    "candidate_slots의 각 항목은 date(YYYY-MM-DD), start_time(HH:MM), end_time(HH:MM), duration_minutes, "
    "reason 필드를 반드시 모두 포함해야 한다. 필드가 하나라도 빠지거나 형식이 안 맞는 후보는 에러 없이 "
    "조용히 결과에서 제외되니, 겹쳐서 빠진 것인지 형식이 틀려서 빠진 것인지 헷갈리지 않도록 필드를 "
    "정확히 채운다. 후보는 busy_rows의 어떤 항목과도 겹치면 안 된다. "
    "이 tool의 결과만으로 답변을 끝내지 말고, 이어서 decide_final_slot을 호출해 최종 시간을 확정해야 한다."
)


DECIDE_FINAL_SLOT_DESCRIPTION = (
    "이 tool은 최종 시간을 자동으로 고르지 않는다. 너(agent)가 candidate_slots 중 하나를 selected_index "
    "또는 selected_slot으로 직접 고르고, final_slot을 'YYYY-MM-DD HH:MM-HH:MM' 형식 문자열로 채워서 "
    "넘겨야 한다. 아직 후보를 확정하지 못했다면 final_slot은 비워두고(null) needs_agent_selection=true로 "
    "넘긴다. 근거 추적을 위해 candidate_slots, busy_rows, member_names, date_from, date_to도 함께 넘긴다."
)
...
def find_common_available_slots_dict(
    ...
    # collect_member_schedules 내부(및 그 아래 MCP 서버)에서도 멤버 이름을 다시 정규화하지만,
    # 여기서 한 번 더 정규화하는 이유는 그 결과가 아니라 "이 함수가 최종적으로 반환하는 payload의
    # members 표시값"을 깨끗하게 만들기 위해서다. 즉 중복 방어가 아니라 서로 다른 목적이다.
    normalized_members = normalize_external_member_names(member_names)
    normalized_date_from = normalize_date_bound(date_from)
    normalized_date_to = normalize_date_bound(date_to)

    # busy_rows=None(안 넘김)과 busy_rows=[](명시적으로 빈 리스트)을 다르게 취급한다.
    # None이면 "아직 조회 안 함"으로 보고 여기서 새로 수집하고, []는 "이미 조회했는데 진짜로
    # 바쁜 시간이 없었다"는 뜻으로 그대로 신뢰한다. tool description이 "이전 조회 결과를 복사해서
    # 넘기라"고 안내하므로, agent가 조회 없이 []를 보내는 경우는 정상 사용 흐름에서 나오지 않는다.
...
def decide_final_slot(
    ...
    # 이 tool은 final_slot이 busy_rows와 실제로 안 겹치는지 다시 검증하지 않는다. agent가 겹치는
    # 시간을 최종으로 골라도 그대로 기록된다 — 가이드가 "nested LLM/Python 규칙으로 재계산하지
    # 말라"고 명시했기 때문에 의도된 신뢰 경계다. 겹침 검증은 find_common_available_slots 단계에서
    # candidate_slots를 만들 때만 한 번 수행된다.
...
def kana_agent(query: str) -> str:
    ...
    # decide_final_slot_payload()(fixed/schedule_decision.py)의 실제 반환 키는
    # final_slot/reason/candidates/needs_agent_selection이고 "final_decision"이라는 키는 없다.
    # 그 이름은 이 파일의 별개 호환용 tool인 propose_group_schedule(kana_tools()에는 포함되지 않음)
    # 에서만 쓰는 이름이라 여기서는 근거가 될 수 없다. 그래서 final_decision_payload는 kana_tools()의
    # 어떤 tool도 그런 키를 안 줘서 항상 None이다 — 필드는 가이드 요구사항대로 유지하되, 실제 값은
    # final_slot_payload(decide_final_slot 결과)로 대신 확인해야 한다.
    final_slot_payload: dict[str, Any] | None = None
    final_decision_payload: dict[str, Any] | None = None
    # events는 시간 순서대로 쌓이므로, "final_slot" 키를 가진 이벤트가 여러 번 나오면(예:
    # decide_final_slot을 후보 부족으로 한 번 더 호출한 경우) 일부러 break 없이 끝까지 돌면서
    # 매번 덮어써 마지막 것을 남긴다. 마지막 매치 = 가장 최근 결정이라는 뜻이라, 덮어쓰는 게 맞다.
    for event in events:
