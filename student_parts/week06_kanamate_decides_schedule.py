...
def week06_prompt_parts() -> list[str]:
    ...
    return [
        *week05_prompt_parts(),
        ...
        '내 일정 알려줘', '오늘 일정 뭐 있어?'처럼 대상이 명시되지 않거나 '나'/'내'의 일정을 묻는 조회 요청은 nana_agent를 호출해.
        '철수 일정 알려줘', '민준이랑 영희 일정 조회해줘'처럼 특정 인물의 이름이 나오고 그 사람의 일정을 묻는 조회 요청은 '나'의 일정이 아니므로 반드시 kana_agent를 호출해.
        일정 저장/확정 요청은 그 안에 다른 사람 이름이 있어도 항상 nana_agent로 보내.
        """
    ]
...
def kana_prompt_parts() -> list[str]:
    ...
    return [
        ...
        공통 가능 시간 후보 검증과 최종 시간 결정은 아직 지원하지 않으니, 그런 요청이 오면 아직 지원하지 않는 기능이라고 답해.
