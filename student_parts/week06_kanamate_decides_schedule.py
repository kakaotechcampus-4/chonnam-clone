...
def week06_prompt_parts() -> list[str]:
    ...
    return [
        *week05_prompt_parts(),
        (
            "너는 Week 6 supervisor다. tool을 직접 호출해 업무를 처리하지 않고, "
            "nana_agent 또는 kana_agent 중 하나에게 위임한 뒤 그 결과만 근거로 답한다.\n"
            "- 개인 일정 생성/조회/수정/삭제, todo/알림 저장, 개인 참고자료·이 앱 대화 검색(RAG)은 "
            "nana_agent에게 위임한다.\n"
            "- 외부 멤버의 이전 대화·일정 조회, 공유 일정 저장소 확인, 여러 사람의 공통 가능 시간 "
            "조율·최종 확정은 kana_agent에게 위임한다.\n"
            "- 한 요청에 개인 업무와 그룹 조율이 섞여 있으면 지금 필요한 하위 작업을 먼저 위임하고, "
            "필요하면 이어서 다른 하위 agent에게 위임한다."
        ),
    ]
...
def nana_prompt_parts() -> list[str]:
    ...
    return [
        *week04_prompt_parts(),
        (
            "너는 Week 6 Nana 하위 agent다. 개인 일정 생성/조회/수정/삭제, todo/알림 저장, "
            "개인 참고자료와 이 앱 대화 검색(RAG)만 담당한다.\n"
            "외부 멤버의 일정이나 여러 사람의 공통 가능 시간 조율은 네 담당이 아니므로, "
            "그런 요청이 들어오면 처리하지 않고 그룹 일정 조율은 Kana 담당이라고 짧게 알린다."
        ),
    ]
...
def kana_prompt_parts() -> list[str]:
    ...
    return [
        (
            "너는 Week 6 Kana 하위 agent다. 외부 멤버의 이전 대화·일정과 공유 일정 저장소, "
            "여러 사람의 공통 가능 시간 조율을 담당한다. 확정된 일정을 이 앱 DB에 저장하는 것은 "
            "네 담당이 아니라 Nana 담당이다.\n"
            "- 자연어 요청을 구조화해야 하면 extract_schedule_request를 사용한다.\n"
            "- 외부 멤버의 과거 대화가 필요하면 search_previous_conversations로 먼저 찾고, "
            "conversation_id로 load_conversation_messages를 이어서 부른다.\n"
...
FIND_COMMON_AVAILABLE_SLOTS_DESCRIPTION = (
    "여러 사람의 busy_rows를 근거로, 네가 직접 고른 공통 가능 시간 후보(candidate_slots)를 "
    "검증하고 기록합니다. 이 tool은 후보를 스스로 계산하지 않습니다 — busy_rows를 먼저 읽고, "
    "그 어떤 busy row와도 겹치지 않는 시간을 네가 직접 골라 candidate_slots로 넘겨야 합니다. "
    "candidate_slots의 각 항목은 date(YYYY-MM-DD), start_time(HH:MM), end_time(HH:MM), "
    "duration_minutes, reason을 포함해야 합니다. busy_rows는 앞서 collect_member_schedules 등으로 "
    "조회한 결과를 그대로 복사해 함께 넘깁니다. member_names/date_from/date_to만 주고 busy_rows를 "
    "비워두면 이 tool이 collect_member_schedules를 대신 호출하며, 그 결과의 external_lookup_ok를 "
    "그대로 반환값에 담아줍니다. external_lookup_ok가 false면 일부 멤버의 busy_rows가 조회 실패로 "
    "불완전하다는 뜻이니, 겹치는 시간이 안 보여도 그 시간이 확실히 비어 있다고 단정하지 마세요. "
    "이 결과만으로 답을 끝내지 말고, 이어서 decide_final_slot을 호출해 최종 시간을 기록하세요."
)


DECIDE_FINAL_SLOT_DESCRIPTION = (
    "find_common_available_slots가 검증한 candidate_slots 중에서, 네가 직접 고른 최종 회의 시간을 "
    "기록합니다. 이 tool은 최종 시간을 자동으로 고르지 않습니다 — selected_index(candidate_slots "
    "안의 index) 또는 selected_slot과 final_slot('YYYY-MM-DD HH:MM-HH:MM' 형식)을 네가 직접 채워 "
    "넘겨야 합니다. 아직 사용자 확인이 더 필요하거나 고를 후보가 없으면 final_slot은 비워두고 "
    "needs_agent_selection=true로 남깁니다. 최종 선택 또는 보류 이유는 reason에 적습니다. "
    "근거 trace를 위해 candidate_slots, busy_rows, member_names, date_from, date_to도 함께 넘기세요."
)
...
def find_common_available_slots_dict(
    ...
    if busy_rows is None:
        ...
        external_lookup_ok = collected.get("external_lookup_ok", True)
        external_lookup_error = collected.get("external_lookup_error")
    ...
    result["external_lookup_ok"] = external_lookup_ok
    result["external_lookup_error"] = external_lookup_error
    return result
...
def kana_agent(query: str) -> str:
    ...
    for event in events:
        ...
        if event.get("tool_name") == "decide_final_slot":
            final_slot_payload = content
        if event.get("tool_name") == "propose_group_schedule" and content.get("final_decision"):
