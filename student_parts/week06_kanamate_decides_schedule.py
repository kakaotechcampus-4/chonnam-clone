...
def week06_prompt_parts() -> list[str]:
    ...
    return [
        *week05_prompt_parts(),
        """
                    너는 supervisor이다. 직접 업무를 처리하지 않는다. 
                    개인 일정 관련된 일이면 nana_agent를 호출하고 개인 일정이 아닌 이외의 일에는 kana_agent를 호출한다. 
                    반드시 둘 중 하나의 agent만 호출하고 그 결과만 근거로 답한다. 
                    """,
    ]
...
def nana_prompt_parts() -> list[str]:
    ...
    return [
        *week04_prompt_parts(),
        """
        이 프롬프트는 Nana 전용 프롬프트이다. 
        개인 일정/저장/RAG를 담당하고, 그룹 조율 요청이 들어온다면 담당이 아니라고 짧게 알려라. 
...
def kana_prompt_parts() -> list[str]:
    ...
    return [
        f"""너는 '카나(Kana)'다. 개인 일정이 아니라 외부 멤버와의 일정 조율을 전담하는 하위 에이전트다.
오늘 날짜는 {current_app_date_iso()}이다. "다음 주 화요일", "이번 주 안에" 같은 상대 날짜 표현은 이 날짜를 기준으로 계산한다.""",
        """네 담당 업무는 다음과 같다:
- 외부 멤버의 일정(busy-time) 조회
- 과거 대화에서 언급된 일정 추출 (필요하면 search_previous_conversations, load_conversation_messages, extract_schedules_from_history 순서로 사용)
- 공유된 일정 row 조회 (list_shared_schedules)
- 멤버별 busy-time을 모아 공통으로 비어 있는 시간 후보를 네가 직접 고르고 검증하기
- 최종 회의 시간을 확정하거나, 확정할 수 없으면 사용자 선택이 필요하다고 남기기""",
        """개인 일정 생성/조회/수정/삭제, todo/reminder 저장, 개인 참고자료나 앱 대화 RAG 검색은 네 담당이 아니다.
이런 요청이 들어오면 "그건 나나(Nana) 담당이야"라고 짧게 답하고 더 진행하지 않는다.""",
    ]
...
FIND_COMMON_AVAILABLE_SLOTS_DESCRIPTION = """
    find_common_available_slots tool은 후보를 계산하지 않는다. 
    1. 먼저 busy_rows를 확인해라 
    2. 겹치지 않는 시간을 최소 1개 이상 찾아라. 
    3. 그 다음에만 tool을 호출해라  
    candidate_slots을 비워서 호출하면 항상 빈 결과만 돌아오고 아무 시간도 찾을 수 없다. 
    그래서 꼭 채우도록 해라 .
    candidate_slots 각 항목이 date(YYYY-MM-DD), start_time(HH:MM), end_time(HH:MM), duration_minutes, reason을 포함해야 한다
    후보는 어떤 busy row와도 겹치면 안 되고, busy_rows도 앞선 tool output에서 복사해 넘긴다.
    이 결과로 답변을 끝내지 말고 decide_final_slot을 이어서 호출한다.   
    
    """


DECIDE_FINAL_SLOT_DESCRIPTION = """
    decide_final_slot은  최종 시간을 자동 선택하지 않는다.
    너는 selected_index 또는 selected_slot과 final_slot을 직접 골라 넘긴다.
    가능하면 selected_index(또는 selected_slot)로 candidate_slots 중에서 골라라.
    후보 목록 자체가 없거나 다른 이유로 직접 텍스트를 써야 한다면 final_slot을 직접 채워도 되지만, 그 경우 검증되지 않은 시간이라는 점을 감안해라
    
    needs_agent_selection은 final_slot이 후보가 하나뿐이고 명확할 때는 False 후보가 여러개가 있거나, 값이 없다면 True를 반환한다.
    reason은 최종 선택 또는 보류에 대한 설명으로 사람이 이해할 수 있도록 자연스러운 문장으로 구성한다. 
    
    
    아직 고르지 않았다면 final_slot은 null, needs_agent_selection은 true로 둔다.
    근거 trace를 위해 candidate_slots, busy_rows, member_names, date_from/date_to도 함께 넘긴다. 
    """
...
def find_common_available_slots_dict(
    ...
    if busy_rows is None:
        ...
        busy_rows = json.loads(raw)["rows"]

    payload_member_names = [*normalized_members, PERSONAL_SHARED_MEMBER_NAME]
