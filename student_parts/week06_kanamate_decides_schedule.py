...
def kana_prompt_parts() -> list[str]:
    ...
    return [
        ...
        - 그룹 일정 조율 순서: search_previous_conversations/collect_member_schedules 등으로
          ...
          마지막으로 decide_final_slot으로 최종 시간을 확정합니다.
        - find_common_available_slots와 decide_final_slot은 시간을 대신 계산해주지 않습니다.
          busy_rows를 직접 읽고 겹치지 않는 후보와 최종 시간을 골라 tool argument로 넘기세요.
        """,
    ]
...
FIND_COMMON_AVAILABLE_SLOTS_DESCRIPTION = (
    "공통 가능 시간 후보를 검증하는 tool입니다. 이 tool은 후보 시간을 스스로 계산하지 않습니다. "
    "먼저 collect_member_schedules 등으로 얻은 busy_rows를 직접 읽고, 겹치지 않는 후보를 candidate_slots에 "
    "직접 담아 넘기세요. candidate_slots 각 항목은 date(YYYY-MM-DD), start_time(HH:MM), end_time(HH:MM), "
    "duration_minutes, reason을 포함해야 합니다. busy_rows는 앞선 tool output에서 그대로 복사해 함께 넘기세요. "
    "이 tool 호출로 답변을 끝내지 말고, 검증된 후보 중 하나를 decide_final_slot으로 이어서 확정하세요."
)


DECIDE_FINAL_SLOT_DESCRIPTION = (
    "최종 회의 시간을 기록하는 tool입니다. 이 tool은 최종 시간을 스스로 고르지 않습니다. "
    "find_common_available_slots가 검증한 candidate_slots 중 하나를 selected_index 또는 selected_slot과 "
    "final_slot('YYYY-MM-DD HH:MM-HH:MM' 형식)으로 직접 골라 넘기세요. "
    "아직 고르지 못했다면 final_slot은 비워 두고 needs_agent_selection=true로 남기세요. "
    "근거를 남기기 위해 candidate_slots, busy_rows, member_names, date_from/date_to, reason도 함께 넘기세요."
)
...
def find_common_available_slots_dict(
    ...
    members_with_me = normalized_members if "나" in normalized_members else [*normalized_members, "나"]
