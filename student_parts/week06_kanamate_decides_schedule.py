...
from fixed.external_people_store import JULY_PRACTICE_MEMBER_NAMES, normalize_external_member_names
...
def week06_prompt_parts() -> list[str]:
    ...
    return [
        *week05_prompt_parts(),
        (
            "너는 Week 6 supervisor다. 업무 tool을 직접 고르지 않고 nana_agent 또는 kana_agent에만 위임한다. "
            "개인 일정, 할 일, 알림, 개인 기억 검색은 Nana에게 맡긴다. "
            "외부 멤버의 지난 대화, 바쁜 시간 조회, 그룹 일정 조율은 Kana에게 맡긴다. "
            "다른 사람 이름이 나오면서 회의 시간을 맞추거나 추천해 달라는 요청은 반드시 Kana에게 맡긴다."
        ),
    ]
...
def kana_prompt_parts() -> list[str]:
    ...
    return [
        (
            "너는 Week 6의 Kana 하위 agent다. 오늘 날짜는 "
            f"{current_app_date_iso()}다. "
            "외부 멤버의 지난 대화 검색, 바쁜 시간 조회, 그룹 회의 가능 시간 판단만 담당한다. "
    ...
    return join_system_prompt(
        [
            *week06_prompt_parts(),
            ...
            "그룹 조율 요청을 Nana의 개인 일정 조회만으로 끝내지 않는다. "
            "Kana가 최종 시간을 제안하면 그 결과를 답하고, 사용자가 저장이나 등록을 명시하지 않았다면 Nana를 추가 호출해 일정을 만들지 않는다. "
            "최종 답변은 하위 agent 결과만 근거로 작성하고 추측하지 않는다.",
        ]
    )
...
DECIDE_FINAL_SLOT_DESCRIPTION = (
    "Kana가 직접 고른 최종 회의 시간을 기록한다. 이 tool은 최종 시간을 대신 선택하지 않는다. "
    "candidate_slots 중 하나를 selected_index 또는 selected_slot으로 고르고 final_slot은 'YYYY-MM-DD HH:MM-HH:MM' 형식으로 채운다. "
    "확정했으면 needs_agent_selection=false, 아직 못 골랐으면 final_slot=null과 needs_agent_selection=true를 넣는다. "
    "근거 trace를 위해 candidate_slots, busy_rows, member_names, date_from, date_to, duration_minutes, reason을 함께 넘긴다."
)
