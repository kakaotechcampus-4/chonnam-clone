...
def kana_agent(query: str) -> str:
    ...
    for event in events:
        ...
        nested_slot = content.get("final_slot_payload")
        if isinstance(nested_slot, dict):
            final_slot_payload = nested_slot
        elif "final_slot" in content:
            final_slot_payload = content

        nested_decision = content.get("final_decision_payload")
        if not isinstance(nested_decision, dict):
            nested_decision = content.get("final_decision")
        if isinstance(nested_decision, dict):
            final_decision_payload = nested_decision
            if "final_slot" in nested_decision:
                final_slot_payload = nested_decision
