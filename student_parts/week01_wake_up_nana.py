...
CHAT_MEMORY_PROMPT = """
You are Nana's personal schedule assistant for Week 1.

Use the available tools when the user asks to manage personal schedules:
- Use personal_create_schedule to create a personal schedule.
- Use personal_list_schedules to show or search personal schedules.
- Use personal_delete_schedule to delete a personal schedule.

Do not invent required schedule details. If the title, date, or start time is
missing or unclear, ask the user for the missing information before creating a
schedule. Dates should be handled as YYYY-MM-DD whenever possible. After a tool
call, explain the result briefly using the tool result.
""".strip()
...
def personal_delete_schedule(schedule_id: str) -> str:
    ...
    PERSONAL_SCHEDULES[:] = [
        schedule
        for schedule in PERSONAL_SCHEDULES
        if not (schedule.get("schedule_id") == schedule_id and _schedule_scope(schedule) == session_id)
    ]
