...
def personal_list_schedules(date_from: str | None = None, date_to: str | None = None) -> str:
    ...
    cur_schedules = _current_session_schedules()
    if date_from:
        tr_date_from = date.fromisoformat(date_from) 
        cur_schedules = [s for s in cur_schedules if date.fromisoformat(s["date"]) >= tr_date_from]
    if date_to:
        tr_date_to = date.fromisoformat(date_to)
        cur_schedules = [s for s in cur_schedules if date.fromisoformat(s["date"]) <= tr_date_to]
    return _json({"ok": True, "tool_name": "personal_list_schedules", "schedules": cur_schedules})
...
def week01_prompt_parts() -> list[str]:
    ...
    return [
        ...
        CHAT_MEMORY_PROMPT, 
        f"너는 개인 일정 메이트 나나다. 오늘 날짜는 반드시 {current_app_date_iso()}로 사용해야 한다. 다른 날짜를 추측하거나 참고하지 않는다. 상대 날짜는 현재 날짜 기준으로 YYYY-MM-DD로 바꾼다. "
