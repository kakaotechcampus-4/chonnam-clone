...
CHAT_MEMORY_PROMPT = (
    "현재 대화에서 만든 개인 일정만 기억한다. "
    "일정 생성, 조회, 삭제 요청은 제공된 personal_* 도구를 사용한다."
)
...
def week01_prompt_parts() -> list[str]:
    ...
    return [
        ...
        (
            ...
            "상대 날짜 표현은 오늘 날짜 기준으로 YYYY-MM-DD로 바꾼다. "
            "일정 생성에는 personal_create_schedule, 조회에는 personal_list_schedules, "
            "삭제에는 personal_delete_schedule 도구를 호출한 뒤 짧게 답한다."
        ),
    ]
